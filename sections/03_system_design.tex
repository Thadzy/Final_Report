...
\begin{table}[H]
  ...
  \small
  \begin{tabular}{>{\raggedright\arraybackslash}p{3.0cm} >{\centering\arraybackslash}p{3.0cm} >{\centering\arraybackslash}p{3.0cm} >{\raggedright\arraybackslash}p{4.5cm}}
    \toprule
...
\begin{table}[H]
  ...
  \small
  \begin{tabular}{>{\raggedright\arraybackslash}p{3.0cm} >{\centering\arraybackslash}p{2.5cm} >{\centering\arraybackslash}p{2.5cm} >{\centering\arraybackslash}p{3.5cm} >{\centering\arraybackslash}p{2.0cm}}
    \toprule
...
% -----------------------------------------------------------------------
\subsection{การคำนวณแรงบิดที่ต้องการและการเลือกมอเตอร์}
\label{subsec:torque_calc}
% -----------------------------------------------------------------------

การคำนวณนี้พิสูจน์ว่าชุดส่งกำลัง (RS-775 + Planetary Gearbox + Belt Drive)
มีแรงบิดเพียงพอที่จะขับแขนกลให้เป็นไปตาม Requirements P3 และ P4
ภายใต้โหลดสูงสุด 2\,kg

\subsubsection{ความเฉื่อยรวมของระบบ}

ความเฉื่อยรวม $J_{\mathrm{total}}$ ได้จากการวิเคราะห์ SolidWorks Model
ครอบคลุมแขนกล, End Effector, และโหลดสูงสุด:

\begin{equation}
  J_{\mathrm{total}} = 0.5764 \; \mathrm{kg \cdot m^2}
  \quad \text{(จาก SolidWorks)}
  \label{eq:jtotal}
\end{equation}

\subsubsection{แรงบิดที่ต้องการ}

แรงบิดที่ต้องการ ($T_{\mathrm{req}}$) คำนวณจากกฎของนิวตันสำหรับระบบหมุน
โดยใช้ความเร่งต่ำสุดที่ระบบต้องทำได้ตาม Requirement P4:

\begin{equation}
  T_{\mathrm{req}} = J_{\mathrm{total}} \times \alpha_{\min}
  = 0.5764 \times 2.0
  = 1.153 \; \mathrm{Nm}
  \label{eq:treq}
\end{equation}
...
\begin{equation}
  T_{\mathrm{final}} = T_{\mathrm{req}} \times SF
  = 1.153 \times 2.0
  = 2.306 \; \mathrm{Nm}
  \label{eq:tfinal}
\end{equation}

\subsubsection{แรงบิดที่ระบบสามารถส่งออกได้จริง}

แรงบิดที่เพลาแขนกลคำนวณจากห่วงโซ่การส่งกำลังทั้งหมด:

\begin{equation}
  T_{\mathrm{arm}} = T_{\mathrm{motor}} \times i_{\mathrm{gear}} \times \eta_{\mathrm{gear}}
                   \times i_{\mathrm{belt}} \times \eta_{\mathrm{belt}}
  \label{eq:tarm_chain}
\end{equation}

\begin{equation}
  T_{\mathrm{arm}} = 0.0955 \times 24 \times 0.85 \times 3 \times 0.95
  = 5.3977 \; \mathrm{Nm}
  \label{eq:tarm}
\end{equation}
...
\begin{table}[H]
  \centering
  \caption{พารามิเตอร์ห่วงโซ่การส่งกำลังสำหรับการคำนวณแรงบิด}
  \label{tab:torque_budget}
  \small
  \begin{tabular}{>{\raggedright\arraybackslash}p{5.5cm}
    ...
    \midrule
    ความเฉื่อยรวม $J_{\mathrm{total}}$ & $0.5764\;\mathrm{kg \cdot m^2}$ & จาก SolidWorks (แขนกล + EE + โหลด) \\
    ความเร่งสูงสุดที่คำนวณได้ $\alpha_{\max}$ & $9.3641\;\mathrm{rad/s^2}$ & มากกว่า P4 ($\leq 28.0\;\mathrm{rad/s^2}$) \\
    ความเร็วสูงสุดที่คำนวณได้ $\omega_{\max}$ & $8.9760\;\mathrm{rad/s}$ & มากกว่า P3 ($\leq 7.5\;\mathrm{rad/s}$) \\
    แรงบิดมอเตอร์ RS-775 $T_{\mathrm{motor}}$ & $0.0955\;\mathrm{Nm}$ & จาก Datasheet (Stall Torque) \\
    อัตราทด Planetary Gearbox $i_{\mathrm{gear}}$ & $24{:}1$ & ภายในชุด Gearmotor \\
    ประสิทธิภาพ Gearmotor $\eta_{\mathrm{gear}}$ & $0.85$ & ค่าเฉลี่ยทั่วไปของ Planetary \\
    แรงบิดหลัง Gearbox $T_{\mathrm{gear}}$ & $1.9481\;\mathrm{Nm}$ & $T_{\mathrm{motor}} \times i_{\mathrm{gear}} \times \eta_{\mathrm{gear}}$ \\
    อัตราทดสายพาน $i_{\mathrm{belt}}$ (70T : 24T) & $3{:}1$ & Timing Belt HTD5M \\
    ประสิทธิภาพสายพาน $\eta_{\mathrm{belt}}$ & $0.95$ & จาก Habasit Catalogue \\
    แรงบิดรวมที่เพลาแขนกล $T_{\mathrm{arm}}$ & $5.3977\;\mathrm{Nm}$ & $T_{\mathrm{gear}} \times i_{\mathrm{belt}} \times \eta_{\mathrm{belt}}$ \\
    \bottomrule
  \end{tabular}
\end{table}

\subsubsection{การตรวจสอบ Safety Factor}

\begin{equation}
  SF_{\mathrm{actual}} = \frac{T_{\mathrm{arm}}}{T_{\mathrm{req}}}
  = \frac{5.3977}{1.153}
  = 4.682
  \label{eq:sf_actual}
\end{equation}

$SF_{\mathrm{actual}} = 4.682 > SF_{\mathrm{req}} = 2.0$ และ
$T_{\mathrm{arm}} = 5.3977\;\mathrm{Nm} > T_{\mathrm{final}} = 2.306\;\mathrm{Nm}$
ดังนั้นระบบส่งกำลัง \textbf{ผ่าน} ข้อกำหนดด้านแรงบิดและ Safety Factor
...
\begin{table}[H]
  ...
  \label{tab:power_24v}
  \small
  \begin{tabular}{>{\raggedright\arraybackslash}p{4.5cm} >{\centering\arraybackslash}p{3.0cm} >{\centering\arraybackslash}p{3.5cm} >{\raggedleft\arraybackslash}p{2.5cm}}
    \toprule
...
\begin{table}[H]
  ...
  \small
  \begin{tabular}{>{\raggedright\arraybackslash}p{3.5cm} >{\raggedright\arraybackslash}p{3.5cm} >{\centering\arraybackslash}p{2.5cm} >{\raggedright\arraybackslash}p{4.0cm}}
    \toprule
...
\begin{table}[H]
  ...
  \small
  \begin{tabular}{>{\raggedright\arraybackslash}p{0.8cm} >{\raggedright\arraybackslash}p{2.4cm} >{\raggedright\arraybackslash}p{2.0cm} >{\raggedright\arraybackslash}p{3.0cm} >{\raggedright\arraybackslash}p{4.8cm}}
    \toprule
...
\begin{table}[H]
  ...
  \small
  \begin{tabular}{>{\centering\arraybackslash}p{2.7cm} >{\centering\arraybackslash}p{2.7cm} >{\centering\arraybackslash}p{2.7cm} >{\centering\arraybackslash}p{2.7cm} >{\centering\arraybackslash}p{2.7cm}}
    \toprule
...
เป็นเส้นตรงที่ $0\degree$ ในทุกความถี่โดยไม่มีข้อยกเว้น
...
\begin{table}[H]
  ...
  \small
  \begin{tabular}{>{\raggedright\arraybackslash}p{4.0cm} >{\raggedright\arraybackslash}p{3.5cm} >{\centering\arraybackslash}p{3.0cm} >{\centering\arraybackslash}p{3.0cm}}
    \toprule
...
\begin{table}[H]
  ...
  \begin{tabular}{>{\raggedright\arraybackslash}p{2.5cm} >{\raggedright\arraybackslash}p{2.5cm} >{\centering\arraybackslash}p{2.0cm} >{\centering\arraybackslash}p{2.0cm} >{\centering\arraybackslash}p{2.5cm} >{\centering\arraybackslash}p{2.0cm}}
    \toprule
    Loop & สัญลักษณ์ & Theoretical & Hardware & หน่วย (Theory) & หน่วย (HW) \\
    \midrule
    \multirow{2}{*}{Velocity (PI)}
      & $K_{p,vel}$ & 68.940   & 1.58  & V/(rad/s)      & PWM/(rad/s) \\
      & $K_{i,vel}$ & 3512.963 & 0.5   & V/rad          & PWM/rad     \\
    \midrule
    \multirow{3}{*}{Position (PID)}
      & $K_{p,pos}$ & 8.889  & 3.00  & (rad/s)/rad     & (rad/s)/rad \\
      & $K_{i,pos}$ & --     & 0.02  & --              & (rad/s)/(rad·s) \\
      & $K_{d,pos}$ & --     & 0     & --              & (rad/s)·s/rad \\
    \midrule
    Anti-windup
      & $K_{t,aw}$  & 50.958 & --    & s$^{-1}$        & s$^{-1}$ \\
    \midrule
    \multirow{3}{*}{Feedforward}
      & $K_{vff}$ & 3.033 & 3.03  & V/(rad/s)         & PWM/(rad/s) \\
      & $K_{aff}$ & 0.445 & 0.10  & V/(rad/s$^2$)     & PWM/(rad/s$^2$) \\
...
\begin{table}[H]
  ...
  \begin{tabular}{>{\raggedright\arraybackslash}p{3.0cm} >{\centering\arraybackslash}p{2.5cm} >{\centering\arraybackslash}p{2.5cm} >{\centering\arraybackslash}p{3.5cm} >{\centering\arraybackslash}p{2.0cm}}
    ...
    \midrule
    Settling Time  & 2.11      & $\leq 0.5$ & s   & ไม่ผ่าน \\
    Overshoot      & 0.00      & $\leq 1$   & \%  & ผ่าน \\
...
%     Steady-state Error   & $0.004\degree$ & $0.004\degree$ & --            & -- \\
%     Rod Peak Deflection  & $21.7\degree$  & $9.9\degree$   & $\leq 0.57\degree$ (at rest) & -- \\
...
\begin{table}[H]
  ...
  \begin{tabular}{>{\raggedright\arraybackslash}p{3.0cm} >{\centering\arraybackslash}p{2.5cm} >{\centering\arraybackslash}p{2.5cm} >{\centering\arraybackslash}p{3.5cm} >{\centering\arraybackslash}p{2.0cm}}
    ...
    \bottomrule
    \multicolumn{5}{l}{\small เกณฑ์: GM $> 6$\,dB และ PM $> 30\degree$} \\
  \end{tabular}
\end{table}
...
ไม่เคยลดลงถึง $-180\degree$ ในย่านความถี่ใด
...
Phase Margin ของ Velocity Loop ($74.6\degree$)
และ Position Loop ($98.1\degree$) สูงกว่าเกณฑ์ขั้นต่ำ
$30\degree$ อย่างมีนัยสำคัญ
...
(Move $360\degree$ ด้วย S-curve ที่ $v_{max} = 7.304$\,rad/s,
...
\begin{figure}[H]
    ...
    \caption{(ก) S-curve Acceleration Profile ของแขนกล
             ...
             สำหรับ Worst-case Move $360\degree$}
...
\begin{table}[H]
  ...
  \begin{tabular}{>{\raggedright\arraybackslash}p{4.5cm} >{\centering\arraybackslash}p{3.0cm} >{\centering\arraybackslash}p{3.0cm} >{\centering\arraybackslash}p{3.0cm}}
    ...
    \midrule
    Rod Peak Deflection  & $179.95\degree$ & $53.52\degree$   & $-70.3\%$ \\
...
ที่ $\pm 180\degree$ เพื่อสะท้อนข้อจำกัดทางกายภาพจริง
อย่างไรก็ตามค่า Peak Deflection ของ Unshaped
ที่ $\approx 180\degree$ บ่งชี้ว่าใน Worst-case
(Move $360\degree$ ด้วย $a_{max} = 27.49$\,rad/s²)
...
\begin{figure}[H]
    ...
    \caption{ลักษณะจำเพาะของ S-Curve Motion Profile สำหรับ 1 การเคลื่อนที่ (Worst-case $360\degree$) 
...
\paragraph{Scenario 1: Worst-case ($0\degree \leftrightarrow 360\degree$)}

แต่ละ Move ใช้ระยะทาง $360\degree$ สลับกันตลอด Cycle
...
\begin{table}[H]
  ...
  \begin{tabular}{>{\centering\arraybackslash}p{2.0cm} >{\raggedright\arraybackslash}p{5.5cm} >{\centering\arraybackslash}p{3.0cm} >{\centering\arraybackslash}p{3.0cm}}
    \toprule
    Move & สถานี & $\Delta\theta$ (deg) & $t_{move}$ (s) \\
    \midrule
    1 & Home $\to$ Pick 1 ($360\degree$)  & $+360$ & 1.146 \\
    2 & Pick 1 $\to$ Place 1 ($0\degree$) & $-360$ & 1.146 \\
...
Pick positions ที่ $45\degree$, $135\degree$, $225\degree$, $315\degree$
และ Place position รวมที่ $270\degree$
...
\begin{table}[H]
  ...
  \begin{tabular}{>{\centering\arraybackslash}p{2.0cm} >{\raggedright\arraybackslash}p{5.5cm} >{\centering\arraybackslash}p{3.0cm} >{\centering\arraybackslash}p{3.0cm}}
    ...
    Move & สถานี & $\Delta\theta$ (deg) & $t_{move}$ (s) \\
    \midrule
    1 & Home ($0\degree$) $\to$ Pick 1 ($45\degree$)    & $+45$  & 0.571 \\
    2 & Pick 1 $\to$ Place 1 ($270\degree$)     & $+225$ & 0.822 \\
    3 & Place 1 $\to$ Pick 2 ($135\degree$)     & $-135$ & 0.720 \\
    4 & Pick 2 $\to$ Place 2 ($270\degree$)     & $+135$ & 0.720 \\
    5 & Place 2 $\to$ Pick 3 ($225\degree$)     & $-45$  & 0.571 \\
    6 & Pick 3 $\to$ Place 3 ($270\degree$)     & $+45$  & 0.571 \\
    7 & Place 3 $\to$ Pick 4 ($315\degree$)     & $+45$  & 0.571 \\
    8 & Pick 4 $\to$ Place 4 ($270\degree$)     & $-45$  & 0.571 \\
    9 & Place 4 $\to$ Home ($0\degree$)         & $-270$ & 0.934 \\
    \midrule
...
\begin{table}[H]
  ...
  \begin{tabular}{>{\raggedright\arraybackslash}p{4.5cm} >{\centering\arraybackslash}p{3.0cm} >{\centering\arraybackslash}p{3.0cm} >{\centering\arraybackslash}p{3.0cm}}
    ...
    \midrule
    Worst-case ($0\degree \leftrightarrow 360\degree$) & 30.41 & $\leq 35$ & \textbf{PASS} \checkmark \\
...
\begin{figure}[H]
    ...
    \caption{Motion Profile ของ Scenario 1 (Worst-case $0\degree \leftrightarrow 360\degree$)
...
เกิน $\phi_{max,allow} = 0.573\degree$ ในระหว่างการเคลื่อนที่
...
\begin{table}[H]
  ...
  \begin{tabular}{>{\raggedright\arraybackslash}p{3.5cm} >{\raggedright\arraybackslash}p{3.5cm} >{\raggedright\arraybackslash}p{3.5cm} >{\raggedright\arraybackslash}p{3.0cm}}
    ...
    0x23 & uint16 & P2P Unit & 0=degree, 1=index (1 index = $5\degree$) \\
