...
\begin{table}[H]
  ...
  \small
  \begin{tabular}{p{1.5cm} p{2.2cm} p{9.8cm}}
    \toprule
    ...
    M9  & Mechanical & หมุนต่อเนื่องได้ $\geq 540\degree$ โดยสายไฟไม่พันกัน \\
    ...
    P3  & Performance & ความเร็วเชิงมุม $\geq 2.9$ rad/s และ $\leq 7.5$ rad/s \\
    P4  & Performance & ความเร่งเชิงมุม $\geq 2.0$ rad/s$^2$ และ $\leq 28.0$ rad/s$^2$ \\
    ...
    A2  & Accuracy & ความแม่นยำซ้ำ 100 รอบ คลาดเคลื่อน $\leq \pm 0.1\degree$ \\
...
\noindent\small{$^*$ ขอบบนของ P3 ($\leq 7.5$ rad/s) และ P4 ($\leq 28.0$ rad/s²)
ปรับปรุงจาก Torque Budget ภายหลัง — ดู\cref{subsec:req_amendment}}

% ----------------------------------------------------------------------------
\subsection{การปรับปรุงข้อกำหนด (Requirement Amendment)}
\label{subsec:req_amendment}

จากการวิเคราะห์ Torque Budget ภายหลัง พบว่าชุดส่งกำลังและมอเตอร์สามารถรองรับความเร็วและความเร่งได้สูงกว่าขีดจำกัดเดิมที่ตั้งไว้แบบ Conservative
ดังนั้นเพื่อให้บรรลุ Cycle Time (P1) จึงปรับขอบบนของ P3 เป็น $v_{max} \le 7.5$ rad/s
และ P4 เป็น $a_{max} \le 28.0$ rad/s$^2$
...
\begin{table}[H]
  ...
  \small
  \begin{tabular}{>{\raggedright\arraybackslash}p{3.0cm} >{\raggedright\arraybackslash}p{6.5cm} >{\raggedright\arraybackslash}p{4.0cm}}
    \toprule
...
% ----------------------------------------------------------------------------
\section{งบประมาณโครงงาน}
\label{sec:budget}

งบประมาณรวมที่ได้รับอนุมัติสำหรับโครงงานนี้คือ \textbf{17{,}000 บาท}
ตารางที่~\ref{tab:budget} สรุปรายการจัดซื้อหลักจำแนกตามหมวดหมู่
(รายการสิ้นเปลืองเล็กน้อย เช่น น็อต สกรู แหวน และสายไฟ รวมไว้เป็นรายการกลุ่ม)

\noindent\textbf{หมายเหตุ:} ยอดรวมในตารางนี้แสดงเฉพาะค่าใช้จ่ายของอุปกรณ์ที่นำมาใช้งานจริงในระบบขั้นสุดท้าย
ไม่รวมค่าใช้จ่ายจากการทดลองและชิ้นส่วนที่เสียหายระหว่างกระบวนการพัฒนา (ค่าลองผิดลองถูก)
...
\begin{longtable}{>{\raggedright\arraybackslash}p{6.5cm}
                  ...
                  >{\raggedleft\arraybackslash}p{1.8cm}}
  \caption{สรุปงบประมาณโครงงาน จำแนกตามหมวดหมู่}
  \label{tab:budget} \\
  \toprule
  ...
  % --- ระบบขับเคลื่อน ---
  \multicolumn{5}{l}{\textbf{ระบบขับเคลื่อน (Drive System)}} \\
  \midrule
  ...
  \midrule
  \textit{รวมระบบขับเคลื่อน} & & & & \textit{3{,}930} \\

  \addlinespace
  % --- Encoder & Sensor ---
  \multicolumn{5}{l}{\textbf{Encoder \& Sensor}} \\
  \midrule
  ...
  \midrule
  \textit{รวม Encoder \& Sensor} & & & & \textit{1{,}930} \\

  \addlinespace
  % --- Electronics & Control ---
  \multicolumn{5}{l}{\textbf{อิเล็กทรอนิกส์และระบบควบคุม}} \\
  \midrule
  ...
  PCB 15$\times$20\,cm + วัสดุประกอบ & 1 & ชุด & 49 & 49 \\
  MOCUTE-052 VR Joystick (Bluetooth) & 1 & ชิ้น & 123 & 123 \\
  XL4016E1 Step-Down Converter 8A & 1 & ชิ้น & 90 & 90 \\
  \midrule
  \textit{รวมอิเล็กทรอนิกส์และระบบควบคุม} & & & & \textit{1{,}392} \\

  \addlinespace
  % --- Power & Safety ---
  \multicolumn{5}{l}{\textbf{ระบบจ่ายไฟและความปลอดภัย}} \\
  \midrule
  ...
  \midrule
  \textit{รวมระบบจ่ายไฟและความปลอดภัย} & & & & \textit{1{,}839} \\

  \addlinespace
  % --- Mechanical ---
  \multicolumn{5}{l}{\textbf{ชิ้นส่วนเชิงกล}} \\
  \midrule
  ...
  Flexible Shaft Coupling 8$\times$8\,mm & 1 & ชิ้น & 130 & 130 \\
  น็อต สกรู แหวน และอุปกรณ์ยึด (รวม) & --- & --- & --- & 600 \\
  บูชรอง (Spacer) ชุด & --- & --- & --- & 299 \\
  \midrule
  \textit{รวมชิ้นส่วนเชิงกล} & & & & \textit{4{,}405} \\

  \addlinespace
  % --- Wiring & Connectors ---
  \multicolumn{5}{l}{\textbf{สายไฟและขั้วต่อ}} \\
  \midrule
  สายไฟ AC/DC, สายสัญญาณ (รวม) & --- & --- & --- & 487 \\
  GX12/GX16/GX20 Connector (รวม) & --- & --- & --- & 450 \\
  Terminal Block, Bus Bar (รวม) & --- & --- & --- & 200 \\
  \midrule
  \textit{รวมสายไฟและขั้วต่อ} & & & & \textit{1{,}137} \\

  \addlinespace
  % --- Miscellaneous ---
  \multicolumn{5}{l}{\textbf{วัสดุสิ้นเปลือง}} \\
  \midrule
  Matte PETG Filament (Black + Blue) & 2 & ม้วน & 263 & 525 \\
  ไส้ไก่, ทองแดง PCB, สารหล่อลื่น (รวม) & --- & --- & --- & 180 \\
  \midrule
  \textit{รวมวัสดุสิ้นเปลือง} & & & & \textit{705} \\

  \addlinespace
  \midrule
  \textbf{รวมทั้งหมด} & & & & \textbf{15{,}338} \\
  \textbf{งบที่ได้รับ} & & & & \textbf{17{,}000} \\
  \textbf{คงเหลือ} & & & & \textbf{1{,}662} \\
  \bottomrule
\end{longtable}
