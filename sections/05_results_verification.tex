...
ซึ่งส่งผลโดยตรงต่อความแม่นยำตำแหน่ง (A1: $\leq \pm 0.5°$)
และความซ้ำซากได้ (A2: $\leq \pm 0.1°$)
...
ล็อกตำแหน่งแขนกลที่มุม $\theta = 0°$, $40°$, $90°$
...
\begin{equation}
    \delta_{backlash} = \frac{\Delta \text{counts}}{8192 \cdot N_{total}} \times 360°
...
\begin{table}[H]
  ...
  \begin{tabular}{l l l l}
    \toprule
    จุดวัด & ระยะคลอน (°) & $\sigma$ (°) & หมายเหตุ \\
    \midrule
    Planetary Gearbox & [VAL\_BACKLASH\_GEAR] & [VAL\_BL\_GEAR\_STD] & วัดที่ Output shaft กล่อง \\
    Belt Drive        & [VAL\_BACKLASH\_BELT] & [VAL\_BL\_BELT\_STD] & วัดที่ Output shaft แขน  \\
    \midrule
    รวมระบบ           & [VAL\_BACKLASH\_TOTAL] & -- &
      เกณฑ์ A2: $\leq \pm 0.1°$ \quad [PASS\_BACKLASH] \\
    \bottomrule
  \end{tabular}
\end{table}
...
หมุนแขนกลผ่านมุม $360°$ หนึ่งรอบเต็ม (ที่ Output Shaft)
...
หรือเทียบได้กับ $\approx 22.76$ counts/° ที่ Output Shaft

\paragraph{การทดสอบ Noise Floor}
ล็อกแขนกลที่ตำแหน่ง $\theta = 0°$ (Brake Enable, PWM = 0)
...
\begin{table}[H]
  ...
  \small
  \begin{tabular}{p{6.0cm} p{3.0cm} p{3.0cm} p{2.0cm}}
    \toprule
    รายการทดสอบ & ผล & เกณฑ์ & สถานะ \\
    \midrule
    Resolution (counts/rev ที่ Motor Shaft) & 8192 & 8192 ($\times 4$ QEI) & ผ่าน \\
    ค่าคงที่ต่อ 1° ที่ Output Shaft         & [VAL\_CNT\_PER\_DEG] & $\approx 22.76$ & [PASS\_CNT] \\
    Noise Peak-to-peak (ขณะหยุดนิ่ง)       & [VAL\_ENC\_NOISE]\,counts & $\leq 2$ counts & [PASS\_NOISE] \\
    Index Pulse ทำงานถูกต้อง                & [VAL\_INDEX\_PASS] & ผ่าน & [PASS\_INDEX] \\
    \bottomrule
  \end{tabular}
\end{table}
...
\paragraph{วัตถุประสงค์}
Proximity Sensor ใช้กำหนดจุด Home ($\theta = 0°$) ทุกครั้งที่เริ่มต้นระบบ
...
\begin{enumerate}
    \item หมุนแขนออกจาก Home ไปยังตำแหน่งสุ่ม $\theta \in [30°, 90°]$
    \item สั่ง Homing Sequence: แขนหมุนกลับด้วยความเร็ว Slow ($\omega = [VAL\_HOMING\_SPD]$\,rad/s)
...
\begin{table}[H]
  ...
  \begin{tabular}{lccc}
    ...
    ความซ้ำซากได้ของ Homing ($\sigma$)    & [VAL\_HOMING\_STD]$°$ & -- & -- \\
    Max Deviation (10 reps)               & [VAL\_HOMING\_REPEAT]$°$ & $\leq \pm 0.1°$ & [PASS\_HOMING] \\
    \bottomrule
  \end{tabular}
\end{table}
...
ให้ Position Loop ส่ง Step Command $\theta_{ref} = 40°$ จากสถานะนิ่ง
...
\begin{table}[H]
  \centering
  \caption{ผลการทดสอบ Motor Driver Response}
  \label{tab:driver_test}
  \begin{tabular}{lccc}
    \toprule
    รายการ & ผล & เกณฑ์ & สถานะ \\
    \midrule
    Peak Current ($i_{a,max}$)    & [VAL\_IA\_PEAK]\,A  & $\leq 10$\,A (Cytron Rated) & [PASS\_IA] \\
    Current Rise Time ($t_{10\text{-}90}$) & [VAL\_IA\_RISE]\,ms & -- & -- \\
    PWM Oscillation              & [VAL\_PWM\_OSC]     & ไม่มี Chatter & [PASS\_PWM] \\
    \bottomrule
  \end{tabular}
\end{table}
...
%     \caption{Motor Driver Response ต่อ Step Position Command $40°$:
...
\begin{table}[H]
  \centering
  \caption{ผลการทดสอบ E-Stop Response Time (5 ครั้ง)}
  \label{tab:estop_test}
  \begin{tabular}{lcccc}
    \toprule
    ครั้งที่ & $t_{stop}$ (ms) & $\omega_{arm}$ ก่อน (rad/s) & ผล E6 \\
    \midrule
    1 & [VAL\_ESTOP\_1] & [VAL\_SPD\_1] & [PASS\_E6\_1] \\
    2 & [VAL\_ESTOP\_2] & [VAL\_SPD\_2] & [PASS\_E6\_2] \\
    3 & [VAL\_ESTOP\_3] & [VAL\_SPD\_3] & [PASS\_E6\_3] \\
    4 & [VAL\_ESTOP\_4] & [VAL\_SPD\_4] & [PASS\_E6\_4] \\
    5 & [VAL\_ESTOP\_5] & [VAL\_SPD\_5] & [PASS\_E6\_5] \\
    \midrule
    เฉลี่ย & [VAL\_ESTOP\_AVG] & -- & [PASS\_E6] \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{table}[H]
  \centering
  \caption{ผลการทดสอบ Modbus RTU Communication (100 Transactions)}
  \label{tab:modbus_test}
  \begin{tabular}{lccc}
    \toprule
    รายการ & ผล & เกณฑ์ & สถานะ \\
    \midrule
    FC03 Round-trip (เฉลี่ย)   & [VAL\_MB\_FC03\_AVG]\,ms & -- & -- \\
    FC03 Round-trip (Max)      & [VAL\_MB\_FC03\_MAX]\,ms & $\leq 50$\,ms & [PASS\_FC03] \\
    FC06 Round-trip (เฉลี่ย)   & [VAL\_MB\_FC06\_AVG]\,ms & -- & -- \\
    FC06 Execution Verified    & [VAL\_MB\_FC06\_VER]     & 100\% & [PASS\_FC06] \\
    CRC Error Rate             & [VAL\_MB\_ERR]\%         & $0\%$ & [PASS\_MB\_ERR] \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{table}[H]
  ...
  \small
  \begin{tabular}{p{3.0cm} p{1.2cm} p{1.2cm} p{2.5cm} p{2.5cm} p{2.0cm} p{1.5cm}}
    \toprule
    พารามิเตอร์ & สัญลักษณ์ & เฉลี่ย & $\sigma$ & Uncertainty (\%) & หน่วย & หมายเหตุ \\
    \midrule
    Back-EMF Constant & $K_b$  & 0.04165 & [VAL\_KB\_STD]  & [VAL\_KB\_UNC]  & V$\cdot$s/rad  & -- \\
    Torque Constant   & $K_t$  & 0.04065 & [VAL\_KT\_STD]  & [VAL\_KT\_UNC]  & N$\cdot$m/A    & -- \\
    Viscous Damping   & $B$    & 0.19279 & [VAL\_B\_STD]   & [VAL\_B\_UNC]   & N$\cdot$m$\cdot$s/rad & สูงสุด \\
    Moment of Inertia & $J$    & 0.72762 & [VAL\_J\_STD]   & [VAL\_J\_UNC]   & kg$\cdot$m$^2$ & -- \\
...
\begin{table}[H]
  ...
  \begin{tabular}{lccc}
    ...
    \midrule
    Chirp 0.5\,Hz & [VAL\_FIT\_05] & [VAL\_NRMSE\_05] & Low-freq dynamics \\
    Chirp 1.0\,Hz & [VAL\_FIT\_10] & [VAL\_NRMSE\_10] & Design frequency \\
    Chirp 2.0\,Hz & [VAL\_FIT\_20] & [VAL\_NRMSE\_20] & Near $f_c$ filter \\
    \bottomrule
  \end{tabular}
\end{table}
...
Phase Margin = 74.64° และ Gain Crossover Frequency = 25.58\,Hz
...
ใน Worst-case Move 360° ด้วย S-curve ที่
...
\begin{table}[H]
  ...
  \caption{สรุปผล Simulation เปรียบเทียบ Unshaped กับ ZVD-Shaped
           (Worst-case Move 360°, มีโหลด Connection Rod)}
...
%     \caption{ผล Simulation ZVD Input Shaper (Worst-case Move 360°):
...
\begin{table}[H]
  ...
  \label{tab:kalman_qr}
  \small
  \begin{tabular}{l p{1.5cm} p{1.5cm} p{7.0cm}}
    \toprule
...
\begin{table}[H]
  ...
  \caption{Steady-State Kalman Gain $\mathbf{K}_\infty$ ที่ใช้ใน Firmware
           (รอยืนยันจาก Firmware Version สุดท้าย)}
  \label{tab:kalman_gain}
  \begin{tabular}{llc}
    \toprule
    สถานะ & สัญลักษณ์ & $K_\infty$ \\
    \midrule
    Position   & $\theta_{arm}$ (rad)   & [VAL\_K\_THETA] \\
    Velocity   & $\omega_{arm}$ (rad/s) & [VAL\_K\_OMEGA] \\
    Current    & $i_a$ (A)              & [VAL\_K\_IA]    \\
    Disturbance& $\tau_d$ (N$\cdot$m)  & [VAL\_K\_TAU]   \\
    \bottomrule
  \end{tabular}
\end{table}
...
ส่ง Step Command $\theta_{ref}$ ขนาด $40°$ จากสถานะนิ่ง
...
\begin{table}[H]
  ...
  \small
  \begin{tabular}{p{3.5cm} p{3.0cm} p{2.5cm} p{2.5cm} p{2.0cm}}
    \toprule
    Metric & ผลจริง & Requirement & เกณฑ์ & สถานะ \\
    \midrule
    Settling Time $t_s$  & [VAL\_TS]\,s  & P5 & $\leq 0.5$\,s & [PASS\_P5] \\
    Overshoot \%OS       & [VAL\_OS]\%   & P6 & $\leq 1\%$    & [PASS\_P6] \\
...
%     \caption{Step Response บน Hardware (With Load, $\theta_{ref} = 40°$):
...
Accuracy Test ทวนสอบข้อกำหนด A1 (Max Error $\leq \pm 0.5°$)
ที่ทุกตำแหน่ง Workstation ทั้ง 4 จุด (0°, 40°, 80°, 120°)
...
\begin{table}[H]
  ...
  \label{tab:accuracy}
  \small
  \begin{tabular}{p{3.0cm} p{3.0cm} p{2.5cm} p{2.5cm} p{2.0cm}}
    \toprule
    ตำแหน่งเป้าหมาย (°) & ค่าจริงเฉลี่ย (°) & Error (°) & $\sigma$ (°) & ผล \\
    \midrule
    0   & [VAL\_POS\_0]   & [VAL\_ERR\_0]   & [VAL\_STD\_0]   & [PASS\_0] \\
    40  & [VAL\_POS\_40]  & [VAL\_ERR\_40]  & [VAL\_STD\_40]  & [PASS\_40] \\
    80  & [VAL\_POS\_80]  & [VAL\_ERR\_80]  & [VAL\_STD\_80]  & [PASS\_80] \\
    120 & [VAL\_POS\_120] & [VAL\_ERR\_120] & [VAL\_STD\_120] & [PASS\_120] \\
    \midrule
...
%          เส้น dashed แนวนอน = ±0.5° (เกณฑ์ A1)
...
%              Mean Error $\pm\sigma$ เทียบกับเกณฑ์ $\pm 0.5°$}
...
Repeatability Test ทวนสอบข้อกำหนด A2 (Max Deviation $\leq \pm 0.1°$)
...
ดำเนิน Full Sequence 100 รอบ: $0^\circ \rightarrow 40^\circ \rightarrow 80^\circ \rightarrow 120^\circ \rightarrow 0^\circ$
...
\begin{table}[H]
  ...
  \small
  \begin{tabular}{p{2.5cm} p{3.0cm} p{2.5cm} p{3.5cm} p{2.0cm}}
    \toprule
    ตำแหน่ง (°) & Mean Error (°) & $\sigma$ (°) & Max Deviation (°) & ผล \\
    \midrule
    40  & [VAL\_REP\_MEAN\_40]  & [VAL\_REP\_STD\_40]  & [VAL\_REP\_MAX\_40]  & [PASS\_REP\_40] \\
    80  & [VAL\_REP\_MEAN\_80]  & [VAL\_REP\_STD\_80]  & [VAL\_REP\_MAX\_80]  & [PASS\_REP\_80] \\
    120 & [VAL\_REP\_MEAN\_120] & [VAL\_REP\_STD\_120] & [VAL\_REP\_MAX\_120] & [PASS\_REP\_120] \\
    \midrule
    Worst Case & -- & -- & [VAL\_REP\_WORST] & เกณฑ์ A2: $\leq \pm 0.1°$ \\
    \bottomrule
  \end{tabular}
\end{table}
...
% เนื้อหา: 3 subplots (แนวตั้ง) สำหรับ 40°, 80°, 120°:
...
%     \caption{Run Chart ความซ้ำซากได้ 100 รอบที่ตำแหน่ง 40°, 80°, 120°
%              เทียบกับเกณฑ์ $\pm 0.1°$ (A2)}
...
\begin{table}[H]
  ...
  \begin{tabular}{lcccc}
    ...
    \midrule
    1 & [VAL\_CT\_1] & [VAL\_MT\_1] & [VAL\_GT\_1] & -- \\
    2 & [VAL\_CT\_2] & [VAL\_MT\_2] & [VAL\_GT\_2] & -- \\
    3 & [VAL\_CT\_3] & [VAL\_MT\_3] & [VAL\_GT\_3] & -- \\
    \midrule
    เฉลี่ย & [VAL\_CT\_AVG] & [VAL\_MT\_AVG] & [VAL\_GT\_AVG] & เกณฑ์ P1: $\leq 35$\,s \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{longtable}{l l l l l}
  ...
  E6  & Electrical & ทดสอบ E-Stop                 & $t_{stop}$ = [VAL\_ESTOP\_AVG]\,ms & [E6\_STATUS] \\
...
จากข้อกำหนดทั้ง 32 ข้อ
ข้อกำหนดหมวด Mechanical (M1--M10) ทั้ง 10 ข้อผ่านการทวนสอบด้วย FEA
และการตรวจสอบจริงในขั้นตอนการสร้าง
ข้อกำหนดหมวด Electrical (E1--E9) และ Joystick (J1) ส่วนใหญ่ทวนสอบด้วย
การตรวจสอบ Firmware และการทดสอบ End-to-End
ข้อกำหนดที่อยู่ระหว่างรอผลการทดสอบเชิงปริมาณได้แก่
P1--P7 (Performance), A1--A2 (Accuracy เชิงตัวเลข)
ซึ่งต้องการข้อมูลจากการทดสอบบน Hardware ที่สมบูรณ์