...
จากนั้นออกแรงกดที่ปลายแขน (ระยะ $r = 500$\,mm จากศูนย์กลาง)
...
\noindent การทดสอบ Motor Driver ยืนยันว่า Cytron MD10C สามารถตาม Current Command
จาก Velocity Loop ได้โดยไม่เกิด Oscillation หรือ Delay ที่ส่งผลต่อ Inner Loop Bandwidth
การทำงานสำเร็จในทุก Cycle Test
...
และ STM32 Controller ยังคงทำงานต่อเนื่อง (ข้อกำหนด E6) การทดสอบสำเร็จทุกครั้ง
...
ตลอดการทดสอบ Full Cycle Run จำนวน 3 รอบ การทดสอบการสื่อสารสำเร็จทุก Transaction
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.88\textwidth]{fig_fft_raw_actual}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.88\textwidth]{fig_crossval_all_runs}
...
\begin{figure}[H]
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{fig_rlocus_velocity}
        \caption{Velocity Inner Loop}
        \label{fig:rlocus_velocity_ch5}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{fig_rlocus_position}
        \caption{Position Outer Loop}
        \label{fig:rlocus_position_ch5}
    \end{subfigure}
    \caption{Root Locus ของ Velocity Inner Loop (ซ้าย) และ Position Outer Loop (ขวา)
             แสดง Desired Poles และ Actual Closed-loop Poles}
...
เนื่องจากติดตั้ง Encoder ที่ Output Shaft ระบบจึงรับรู้ตำแหน่งจริงเสมอ
ความคลาดเคลื่อน 0.143 องศาไม่ได้เกิดจาก Backlash โดยตรง
แต่เกิดจากข้อจำกัดของตัวควบคุม (Controller) ที่ไม่สามารถสั่งจ่ายแรงดันให้มอเตอร์เอาชนะแรงเสียดทานสถิต (Static Friction) และ Dead-band ในจังหวะสุดท้ายได้หมด
ทำให้เกิด Steady-state Error เล็กน้อย
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{fig_cycletime_line}
    \caption{Robot Motion Profile (Position \& Velocity) ทั้ง 3 รอบ:
             รอบที่ 1 (น้ำเงิน, $\approx$30.5\,s), รอบที่ 2 (แดง, $\approx$36.8\,s — เกินเกณฑ์)
             และรอบที่ 3 (ส้ม, $\approx$33.9\,s) แสดงตำแหน่ง (บน) และความเร็ว (ล่าง)
             ตลอด Cycle เต็มรูปแบบ}
    \label{fig:cycletime_line}
\end{figure}

\noindent\textbf{หมายเหตุ P1:}
แม้ค่าเฉลี่ยจะอยู่ที่ 32.2 s แต่ในรอบที่ 2 เกิดความล่าช้า (36.8 s)
ซึ่งละเมิด Hard Real-time Constraint คาดว่าเกิดจาก Latency ของ OS Interrupt
หรือ Modbus Communication ดังนั้น P1 จึงมีสถานะ \textbf{ไม่ผ่าน}
...
\begin{longtable}{
  ...
  P3  & Performance & วัดความเร็วจาก Encoder   & $v_{max}$ = 7.3\,rad/s (เกณฑ์ปรับเป็น $\leq 7.5$) & ผ่าน \\
  P4  & Performance & วัดความเร่งจาก Encoder   & $a_{max}$ = 27\,rad/s$^2$ (เกณฑ์ปรับเป็น $\leq 28.0$) & ผ่าน \\
...
\begin{itemize}
    \item \textbf{ผ่าน:} M1-M10, P2-P4, P6, P7, A1, A3-A5, E1-E9, J1
    \item \textbf{ไม่ผ่าน:} P1, P5, A2
\end{itemize}